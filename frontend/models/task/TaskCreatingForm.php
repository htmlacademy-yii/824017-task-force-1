 ...
 */
class TaskCreatingForm extends \yii\db\ActiveRecord
{
    ...
    public function rules()
    {
        return [
            ...
            /*['name', 'match', 'pattern' => "/(?=(.*[^ ]){10,})/", 'message' => 'Длина поля «{attribute}» должна быть не меньше 10 не пробельных символов'],
            ['description', 'match', 'pattern' => "/(?=(.*[^ ]){30,})/", 'message' => 'Длина поля «{attribute}» должна быть не меньше 30 не пробельных символов'], этот паттерн работает. Но страница зависает после повторной проверки..не смог решить эту проблему(*/
            [['name', 'description', 'specialization_id', 'payment', 'deadline_date'], 'safe'],
            [['specialization_id'], 'exist', 'skipOnError' => true, 'targetClass' => Specializations::className(), 'targetAttribute' => ['specialization_id' => 'id'], 'message' => 'Такой специализации не существует'],
            ['payment', 'integer', 'min' => 1, 'message' => 'Значение должно быть целым положительным числом', 'tooSmall' => 'Значение должно быть целым положительным числом'],
